...
if st.button('Predict Salary',type='primary'):
    input_data = input_data = np.array([[age, gender_male, edu_value, job_mean]])
    prediction = model.predict(input_data)

    st.markdown('---')
    st.subheader(f'Estimated Salary: ${prediction[0]:.2f} per year')
    st.write(f"Analysis completed for a **{age}-year-old** **{job_choice}** holding a **{edu_choice}** degree.")
